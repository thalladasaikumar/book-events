...
const validation = [
  ...
  check('zip').trim().not().isEmpty()
    ...
    .withMessage('Zip should be numeric').isLength({min:5, max:5})
...
async function handleValidationErrors(req, res, next){
  ...
  if(!errors.isEmpty()){
    ...
    let usernameCheck = await new userDB().getUser(req.body.username);
    if(usernameCheck.length>0){
      if(req.body.username === usernameCheck[0].userId && req.body.email === usernameCheck[0].emailAddress){
        return res.render('signup',{session:undefined, errorMsg: new Array({msg:'Username and email is already taken'})});
...
router.post('/newConnection', urlencodedParser, [
  check('connection_name','Connection name should be minimum length 5 characters').trim().not().isEmpty().isLength({min:5}).escape(),
  check('connection_category','Connection Category should be minimum 2 characters').trim().not().isEmpty().isLength({min:2}).escape(),
