...
router.post('/savedConnections*', urlencodedParser, assignSession, async function(req, res){
  try{
    ...
    if(!action){
      res.redirect('savedConnections');
    } else if(/^\d+$/.test(connid) && connectionResponse &&  (await connectionDBObj.getConnection(connid)).getConnectionId!=undefined){
      if(action == 'save' || action == 'update' || action == 'delete'){
        ...
        const connectionDetails = await connectionDBObj.getConnection(connid);
        if(action == 'save'){
